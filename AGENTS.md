...
`session-lattice.d.ts` at the repo root is the public API doc for the HTTP reads surface - the practical analog of OpenAPI for an HTTP service. It is auto-generated by `scripts/emit_dts.py` from the FastAPI reads app's OpenAPI schema (`create_reads_app` in `session_lattice/service.py`), and a pre-commit hook regenerates it on every commit so it cannot drift. Consumers can read it without running anything locally.

Do not edit `session-lattice.d.ts` by hand. Add routes to `_register_read_routes()`, the hook updates the spec. Per-route doc lines come from the handler's docstring (FastAPI's `description`/`summary` fields), so write a real `"""..."""` docstring on each handler rather than a leading `#` comment. Manual regen: `coily exec emit-dts`.
